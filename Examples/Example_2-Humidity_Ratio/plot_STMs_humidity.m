...
%     plot(highlight_y, highlight_z, 'ko', 'MarkerSize', 12, 'MarkerFaceColor', 'yellow'); % Use a yellow circle
%     hold off;
% end

%%===================================New set for 8 figures ===
figure;
numRows = 2; % Number of rows for subplot grid
numCols = 3; % Number of columns for subplot grid

% Set up tiled layout with tighter spacing
tttt = tiledlayout(numRows, numCols, 'Padding', 'compact', 'TileSpacing', 'compact');
...
for k = 1:numModes
  ...
  % subplot(numRows, numCols, (subplotRow - 1) * numCols + subplotCol);
     nexttile;
    imagesc(data.y, data.z, flipud(reshap_mode_i)); % Use transpose for correct orientatio  % flip
    hold on; % Hold the current plot to overlay quiver on top
    colormap(brighten(redblueTecplot(21),-0.05)); % Apply colormap
    colorbar; % Add colorbar for reference
   ...
   % ylabel("z", 'FontSize', 20); 
    title(['Spatial Mode ', num2str(i)], 'FontSize', 20);
  %  set(gca, 'FontSize', 17);
...
% Save figure with reduced margins
...
% Compute Vspdmd
%Hspdmd = DMDModes_xsp * time_dynamics;  % sorted 
Hspdmd =Phi*diag(answer.xsp(:,kk))*Vand; % unsorted 
...
% Compute Vdmd 
Hdmd = DMDModes_xdmd * time_dynamics_Org; % sorted 
%Hdmd = Phi * diag(xdmd)*Vand; 
% Create a single figure for combined plots

%% compute Companion DMD
% H_org =Hdata(:,1:51);
% [ HKModes,HKEv,HKAmps,HNorms,HCdmd ] = CompanionMatrix_DMD( H_org );
...
plot(t, abs(Hdmd(highlight_index, :)), 'LineWidth', 2, 'DisplayName', ['DMD-Recon y_k,i ', num2str(highlight_index)]);
 hold on;

plot(t, abs(Hspdmd(highlight_index, :)), 'LineWidth', 2, 'DisplayName', ['SPDMD-Recon y_k,i ', num2str(highlight_index)]);
hold on;
...
mean_Vdmd = mean(abs(Hdmd), 1);   % Mean for Vdmd over 3880 trajectories
std_Vdmd = std(abs(Hdmd), 0, 1);  % Standard deviation for Vdmd
...
mean_Vspdmd  = mean(abs(Hspdmd ), 1);   % Mean for Vspdmd over 3880 trajectories
std_Vspdmd  = std(abs(Hspdmd), 0, 1);  % Standard deviation for Vspdmd
...
rmse_Vdmd = sqrt(mean((real(V0(highlight_index, :) - Hdmd(highlight_index, :))).^2, 1))/ sqrt(length(t));

% Compute RMSE over time for SPDMD-reconstructed data
rmse_Vspdmd = sqrt(mean((real(V0(highlight_index, :) - Hspdmd(highlight_index, :))).^2, 1))/ sqrt(length(t));
...
% Case 2: Vdmd
mean_Vdmd = mean(real(Hdmd), 1);   % Mean for Vdmd over 3880 trajectories
std_Vdmd = std(real(Hdmd), 0, 1);  % Standard deviation for Vdmd
...
% Case 4: Vspdmd
mean_Vspdmd = mean(real(Hspdmd), 1);   % Mean for Vspdmd over 3880 trajectories
std_Vspdmd = std(real(Hspdmd), 0, 1);  % Standard deviation for Vspdmd
