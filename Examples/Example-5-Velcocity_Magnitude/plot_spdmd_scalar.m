...
title(' Koopman Eigenvalues on SCALE', 'FontSize', 16);
...
semilogy(growth_rates, imag(log(Edmd)), 'bo', ... % All modes
     sp_growth_rates, imag(log(Edmd(ival))), 'r+', ... % Sparsity-level modes
     'LineWidth', 1, 'MarkerSize', 7);
...
%Option: qsue quiver or imagesc
% for i = 1:2:rr
%     figure; % Create a new figure for each mode
%         subplotRow = ceil(i / numCols);
%     subplotCol = mod(i - 1, numCols) + 1;
%     % Prepare magnitude data for imagesc
%     mode_i = abs(DMDModes_xsp(:, i)); 
%     mode_i = mode_i(1:40*97); % Extract the first 40*97 elements
%     mode_i_reshaped = reshape(mode_i, [40, 97])'; % Reshape to 40x97 grid
% %subplot(numRows, numCols, (subplotRow - 1) * numCols + subplotCol);
%     % Plot the background heatmap using imagesc
%     imagesc(data.y, data.z, mode_i_reshaped);  
%     hold on; % Hold the current plot to overlay quiver on top
%     colormap(brighten(redblueTecplot(21),-0.55)); % Apply colormap
%     colorbar; % Add colorbar for reference

    % Generate grid for quiver plot
  %  [Y, Z] = meshgrid(data.y, data.z); 

    % Compute directional components using gradients
    %[VV, UU] = gradient(mode_i_reshaped); % VV = d/dy, UU = d/dz
    
    % Overlay quiver plot on top of imagesc
   % quiver(Y, Z, UU, VV,'k'); % 'k' sets the arrows to black for visibility
...
%     hold off; % Release the plot hold for next iteration
% end

% % use imagesc: spDMD modes w.r.t. the order of eigvalues `DEv_xsp'
% for i = 1:2:rr
%     figure; % Create a new figure for each mode
...
for i = 1:2:rr
    ...
    xlabel('Time');
    ylabel('Evolution $Re(\lambda_j^{k}b_{j})$');
    % Scale the y-axis tick labels
yt = yticks; % Get current y-tick positions
%yticklabels(string(yt / 10)); % Divide tick labels by 10
...
%Vspdmd = DMDModes_xsp*time_dynamics;  % sorted with amplitueds
...
% Adjust the axes to "tighten" the figure
axis tight;  % Makes the axis fit tightly around the data

% Ensure the plot fits tighter on both sides by setting limits
xlim([min(t), max(t)]);  % Set x-axis limits manually to control the side margins

% Adjust the figure size
set(gca, 'LooseInset', get(gca, 'TightInset'));  % Removes extra white space around the plot
set(gcf, 'Renderer', 'painters');
...
% fprintf(' Mode  Index_Mode  Norm_xsp  DEv_xsp  Norm_Eigvals  E-Folding Time   Period\n');
% fprintf('------------------------------------------------------------------------------------------\n');
% for i = 1:2:num_eigenvalues
%  fprintf('%5d   %.2f          %.2f           %.3f + %.3fi\n         %.2f        %.2f         %.2f\n', ...
%            i,  Index_xsp(i),  Norm_xsp(i),  DEv_xsp(i) ,norms(i),  e_folding_times(i), periods(i));
% end

% Display results
% results_table = table((1:num_eigenvalues)', norms, e_folding_times, periods, ...
%     'VariableNames', {'Mode', 'Norm', 'E_Folding_Time', 'Period'});
% disp(results_table);
