...
elseif p.Results.type=="Lorentzian"
    ...
    % elseif p.Results.type == "Radial"
    % % Radial Basis Function (RBF) Kernel
    % d = mean(vecnorm(Xa-mean(Xa,2)));
    % kernel_f = @(x,y) exp(-norm(x - y)^2 / (2 * d^2));  % Gaussian RBF
    elseif p.Results.type == "Polynomial"
    % Polynomial kernel
    degree = 3; % Degree of the polynomial, can be modified
    c = 1;      % Constant term (coefficient), can be modified
    kernel_f = @(x,y) (x' * y + c).^degree;
end
