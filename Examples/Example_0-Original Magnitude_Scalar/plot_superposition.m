...
hold on;
plot(t, abs(Vdmd(highlight_index, :)), 'LineWidth', 2, 'DisplayName', ['DMD-Superpoition y_k,i ', num2str(highlight_index)]);
hold on;
plot(t, abs(Vspdmd(highlight_index, :)), 'LineWidth', 2, 'DisplayName', ['SPDMD-Superpoition y_k,i ', num2str(highlight_index)]);

% Customize the plot
title('Superposition Comparison');
xlabel('Time');
ylabel('Real Part of Superposition Evolution');
