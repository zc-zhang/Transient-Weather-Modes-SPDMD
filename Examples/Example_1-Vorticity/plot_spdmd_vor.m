...
scatter(real(Edmd), imag(Edmd), 75, abs(Edmd), 'filled');  % '50' sets the marker size, 'abs(Edmd)' provides color by magnitude
hold on;
...
%semilogy(imag(log(Edmd)),abs(xdmd),'ko', ...
 ...
 plot(real(log(Edmd)),abs(xdmd),'ko', ...
      real(log(Edmd(ival))),abs(answer.xpol(ival,kk)),'r+', ...
     'LineWidth',1,'MarkerSize',7)
xlab = xlabel('real part','interpreter','tex')
set(xlab,'FontName','cmr10','FontSize',26)
ylab = ylabel('amplitude','interpreter','tex')
set(ylab,'FontName','cmr10','FontSize',26)
h = get(gcf,'CurrentAxes'); 
...
%     mode_i_reshaped = reshape(mode_i, [40, 97])'; % Reshape and transpose for correct orientation
% 
%     % Create a next tile in the tiled layout
%     nexttile;
...
%   %  set(gca, 'FontSize', 17);
% 
%     % Make axis and plot box tighter
%  %   axis tight;
% end
% 
