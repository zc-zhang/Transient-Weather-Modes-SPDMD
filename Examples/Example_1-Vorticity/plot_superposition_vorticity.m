...
Vspdmd_r_17 = Phi*diag(answer.xsp(:,69))*Vand;
Vspdmd_r_23 = Phi*diag(answer.xsp(:,51))*Vand;
Vspdmd_r_30 = Phi*diag(answer.xsp(:,1))*Vand;
%Vspdmd_r_30_xpol = Phi*diag(answer.xpol(:,1))*Vand;
...
plot(t, abs(V0(highlight_index, :)), 'b', 'LineWidth', 3, 'DisplayName', ['Original ${\bf{y}}_{k,{\rm{i}}}$, Index ', num2str(highlight_index)]);
hold on;
plot(t, abs(Vdmd(highlight_index, :)), 'LineWidth', 3, 'DisplayName', ['DMD ${\bf{y}}_{k,{\rm{i}}}$ ', num2str(highlight_index)]);
hold on;
plot(t, abs(Vspdmd(highlight_index, :)), 'LineWidth', 3, 'DisplayName', ['SPDMD ${\bf{y}}_{k,{\rm{i}}}$ ', num2str(highlight_index)]);
hold on;
plot(t, abs(Vspdmd_r_17(highlight_index, :)), 'LineWidth', 3, 'DisplayName', ['SPDMD ${\bf{y}}_{k,{\rm{i}}}$', num2str(highlight_index)]);
hold on;
plot(t, abs(Vspdmd_r_23(highlight_index, :)), 'LineWidth',3, 'DisplayName', ['SPDMD ${\bf{y}}_{k,{\rm{i}}}$', num2str(highlight_index)]);
hold on;
plot(t, abs(Vspdmd_r_30(highlight_index, :)), 'LineWidth', 3, 'DisplayName', ['SPDMD ${\bf{y}}_{k,{\rm{i}}}$ ', num2str(highlight_index)]);
%hold on
%plot(t, abs(Vspdmd_r_30_xpol(highlight_index, :)), 'LineWidth', 3, 'DisplayName', ['SPDMD ${\bf{y}}_{k,{\rm{i}}}$ ', num2str(highlight_index)]);
